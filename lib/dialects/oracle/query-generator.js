...
module.exports = (function () {
  var QueryGenerator = {
    ...
    createTableQuery: function (tableName, attributes, options) {
      ...
      if (!!options.uniqueKeys) {
        Utils._.each(options.uniqueKeys, function (columns, indexName) {
          ...
          values.attributes += ', UNIQUE ' + ' (' + self.quoteIdentifier(indexName) + ')';
    ...
    insertQuery: function (table, valueHash, modelAttributes, options) {
      ...
      // console.log('sql: ', Utils._.template(query)(replacements));
      return {
    ...
    selectQuery: function (tableName, options, model) {
      ...
      mainAttributes = mainAttributes && mainAttributes.map(function (attr) {
        ...
        if (Array.isArray(attr) && attr.length === 2) {
          ...
          attr = [attr[0], self.quoteIdentifier(attr[1], true)].join(' '); // Fixed ORA-00923 (Force quote)
        } else {
      ...
      if (options.include) {
        var generateJoinQueries = function (include, parentTable) {
          ...
          if (tableName !== parentTable && mainTableAs !== parentTable) {
            //as = parentTable + '.' + include.as; // ORA-00905: "missing keyword"
            as = include.as;
          }
          ...
          if (through) {
            var throughTable = through.model.getTableName(),
              //throughAs = as + '.' + through.as, //message --> "ORA-00907" Error num --> 907
              throughAs = through.as,
            ...
            if (self._dialect.supports.joinTableDependent) {
              ...
              if (include.include && joinType === ' LEFT OUTER JOIN ') joinQueryItem += '<%= ( %>';// Fix includes left
              ...
              if (include.include && joinType === ' LEFT OUTER JOIN ') joinQueryItem += '<%= IN %>';// Fix includes left
          ...
          if (include.include) {
            include.include.forEach(function (childInclude) {
              ...
              if (childJoinQueries.mainQuery) {
                if (joinQueries.mainQuery[0].indexOf('LEFT OUTER JOIN (') > 0) {//Fix includes left
                  var joinQueryLeft = joinQueryItem.replace('<%= ( %>', '(');
                  joinQueryLeft = joinQueryLeft.replace('<%= IN %>', ')'+childJoinQueries.mainQuery);
                  joinQueries.mainQuery[0] = joinQueryLeft; 
                } else {
    ...
    attributeToSQL: function (attribute) {
      ...
      if (attribute.type instanceof DataTypes.ENUM) {
        ...
        // console.log(attribute.type.toString);
    ...
    */
    ...
    upsertQuery: function (tableName, field, updateField, checkedFields) {
      let checkedFieldSql = checkedFields.args.map(arg => {
          let queryEls = Utils._.map(arg, (val, key) => {
              return key + "=" + "'" + val + "'";
            }),
            query = queryEls.join(' AND ');

          return query;
        }).join(' OR '),
        checkedFieldNames = checkedFields.args.map(arg => {
          return Utils._.map(arg, (val, key) => {
              return key ;
            });
        }).join(' OR '),
        getFieldsSET = Utils._.compact(Utils._.map(updateField, (val, key) => {
          if (checkedFieldNames.includes(key)) {// "ORA-01747" if not condition
            return null;
          }
          return key + '=' + this.escape(val);
        })).join(','),
        getMatchedClause = getFieldsSET !== "" ? `WHEN MATCHED THEN UPDATE SET ${getFieldsSET}` : "",
        upsertQuery = `
        MERGE INTO ${tableName} USING DUAL ON ( ${checkedFieldSql} ) 
        WHEN NOT MATCHED THEN
        INSERT ( ${Utils._.keys(field).join(',')} )
        VALUES
        	( ${Utils._.map(field, (val, key) => {
